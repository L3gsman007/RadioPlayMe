class RadioPlayer {
  constructor() {
    ...
    // LED handles (optional if already present in HTML)
    this.ledRed = document.getElementById("status-led-red");
    this.ledGreen = document.getElementById("status-led-green");
    this._updateRadioStatus(false);
    ...
    if (this.themeToggleBtn && this.themeIcon) {
      ...
      const applyTheme = (t) => {
        ...
        if (t === "light") {
          header && header.classList.add("bg-light", "navbar-light");
          header && header.classList.remove("bg-dark", "navbar-dark");
          document.querySelectorAll(".btn").forEach((b) => {
            b.classList.remove("btn-outline-light");
            b.classList.add("btn-outline-dark");
          });
          this.themeIcon && (this.themeIcon.className = "fas fa-sun");
        } else {
          header && header.classList.add("bg-dark", "navbar-dark");
          header && header.classList.remove("bg-light", "navbar-light");
          document.querySelectorAll(".btn").forEach((b) => {
            b.classList.remove("btn-outline-dark");
            b.classList.add("btn-outline-light");
          });
          this.themeIcon && (this.themeIcon.className = "fas fa-moon");
        }
      };
      applyTheme(theme);
  ...
  _updateRadioStatus(isPlaying) {
    if (this.ledGreen && this.ledRed) {
      this.ledGreen.style.display = isPlaying ? "inline-block" : "none";
      this.ledRed.style.display = isPlaying ? "none" : "inline-block";
    }
  }
  ...
  // Helpers for LED based on play state
  _updateRadioStatus(isPlaying) {
    if (this.ledGreen && this.ledRed) {
      this.ledGreen.style.display = isPlaying ? "inline-block" : "none";
      this.ledRed.style.display = isPlaying ? "none" : "inline-block";
  ...
  initializeElements() {
    ...
    this.streamInfoToggleBtn = document.getElementById(
    ...
    this.extendedConnectionStatus = document.getElementById(
  ...
  setupEventListeners() {
    ...
    this.streamInfoToggleBtn.addEventListener("click", () =>
      this.toggleStreamInfoSection()
    );
    this.stationsToggleBtn.addEventListener("click", () =>
      this.toggleStationsSection()
    );
  ...
  async searchStations() {
    ...
    if (streamUrl) {
      ...
      try {
        ...
        this.showError(
          "Please enter a valid stream URL (e.g., https://cvtfradio.net:8090)"
        );
        return;
      }
    }
    ...
    if (!query && !genre && !country) {
      this.showError(
        "Please enter a search term, select a genre/country, or provide a stream URL"
      );
      return;
    }
  ...
  displayStations(stations, title) {
    ...
    const stationsHTML = stations
      .map((station) => this.createStationHTML(station))
      .join("");
    ...
    const favoritesHTML = favorites
      .map((station) => {
        return `
                <div class="row mb-3">
                    <div class="col">
                        <div class="card station-card" style="cursor: pointer;" data-station='${JSON.stringify(
                          station
                        )}'>
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <button class="btn btn-outline-danger btn-sm remove-favorite" data-station-id="${
                                      station.id
                                    }">
  ...
  createStationHTML(station) {
    const serverTypeBadge =
      station.server_type && station.server_type !== "Unknown"
                                ...
                                ${
                                  station.favicon
                                    ? `<img src="${this.escapeHtml(
                                        station.favicon
                                      )}" alt="" class="me-3" style="width: 32px; height: 32px;" onerror="this.style.display='none'">`
                                    : ""
                                }
                                <div class="flex-grow-1">
                                    <div class="d-flex align-items-center mb-1">
                                        <h6 class="card-title mb-0 me-2">${this.escapeHtml(
                                          station.name
                                        )}</h6>
                                        ${serverTypeBadge}
                                    </div>
                                    <small class="text-muted">
                                        ...
                                        ${
                                          station.country
                                            ? this.escapeHtml(station.country)
                                            : "Unknown Country"
                                        }
                                        ${
                                          station.bitrate
  ...
  async toggleFavorite() {
    ...
    try {
      if (isFavorite) {
        const favoriteStation = this.favorites.find(
          (f) => f.url === this.currentStation.url
        );
        if (favoriteStation) {
  ...
  updateMetadata() {
    if (this.currentStation) {
      ...
      if (this.currentStation.bitrate)
        info.push(`${this.currentStation.bitrate}kbps`);
      if (this.currentStation.codec)
        info.push(this.currentStation.codec.toUpperCase());

      this.currentMetadataEl.textContent =
        info.length > 0 ? info.join(" • ") : "Now Playing";
    }
  }
  ...
  handleAudioError(e) {
    ...
    if (this.audio.error) {
      switch (this.audio.error.code) {
        ...
        default:
          errorMessage =
            "Stream unavailable - this is common with internet radio";
      }
    }
  ...
  initializeMetadataDisplay(station) {
    ...
    this.streamBitrateEl.textContent = station.bitrate
      ? `${station.bitrate} kbps`
      : "Unknown";
    this.streamFormatEl.textContent = station.codec
      ? station.codec.toUpperCase()
      : "Unknown";
  ...
  parseStreamStats(data, url) {
    try {
      ...
      if (data.includes("<SONGTITLE>")) {
        ...
        const listenersMatch = data.match(
  ...
  getStationNameFromUrl(url) {
    try {
      ...
      if (hostname.includes("radio"))
        return hostname.split(".")[0].toUpperCase() + " Radio";

      return (
        hostname.replace("www.", "").split(".")[0].toUpperCase() + " Stream"
      );
    } catch (e) {
  ...
  toggleStreamInfoSection() {
    ...
    if (!isVisible) {
      this.hideWelcomeCanvas();
      this.streamInfoSection.scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
    }
  ...
  toggleStationsSection() {
    ...
    if (!isVisible) {
      ...
      this.stationsSection.scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
    }
  ...
  updateExtendedMetadata() {
    ...
    this.extendedSampleRate.textContent =
      this.currentStation.samplerate || "Unknown";
  }
  ...
  async addFavorite(station) {
    try {
      ...
      if (response.status === 401) {
        this.showError("Please log in to manage favorites");
        return;
      }
  ...
  async addToRecentlyPlayed(station) {
    try {
      ...
      if (response.status === 401) {
        // Optional: prompt login or ignore
        return;
      }
